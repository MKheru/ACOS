...
def quick_validate(candidate_path: Path, python: Path) -> tuple[bool, str]:
    ...
    code = (
        ...
        "    except Exception as e:\n"
        "        print(f'CALL_FAIL: {type(e).__name__}: {e}', file=sys.stderr); sys.exit(3)\n"
        "elif hasattr(m, 'MCPReputationRegistry'):\n"
        "    if not (hasattr(m, 'IncidentSeverity') and hasattr(m, 'MCPIdentity')):\n"
        "        print('MISSING_REPUTATION_TYPES', file=sys.stderr); sys.exit(2)\n"
        "    try:\n"
        "        reg = m.MCPReputationRegistry()\n"
        "        ident = m.MCPIdentity(uuid='u1', name='x', install_origin='', declared_capabilities=frozenset(['read']))\n"
        "        reg.register(ident)\n"
        "        reg.evaluate_action('u1', 'tool_call', {'requested_capability': 'read'})\n"
        "    except Exception as e:\n"
        "        print(f'CALL_FAIL: {type(e).__name__}: {e}', file=sys.stderr); sys.exit(3)\n"
        "elif hasattr(m, 'InstallValidator') and hasattr(m, 'EnvelopeVerifier'):\n"
        "    if not (hasattr(m, 'InstallManifest') and hasattr(m, 'InstallContract') and hasattr(m, 'Envelope')):\n"
        "        print('MISSING_INSTALL_TYPES', file=sys.stderr); sys.exit(2)\n"
        "    try:\n"
        "        validator = m.InstallValidator()\n"
        "        manifest = m.InstallManifest(name='x', version='1', install_origin='npm:x')\n"
        "        validator.validate_manifest(manifest)\n"
        "        verifier = m.EnvelopeVerifier(b'secret')\n"
        "        env = verifier.wrap(b'p', 'u1', 'host', 'read_url')\n"
        "        verifier.verify(env, b'p')\n"
        "    except Exception as e:\n"
        "        print(f'CALL_FAIL: {type(e).__name__}: {e}', file=sys.stderr); sys.exit(3)\n"
        "else:\n"
