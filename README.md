...
- **Phase 2**: Rust-native LLM runtime via [mistral.rs](https://github.com/EricLBuehler/mistral.rs) + [Gemma 4](https://blog.google/innovation-and-ai/technology/developers-tools/gemma-4), then internal Guardian -- on-device AI engine embedded in ACOS, no external dependency. The Web GUI remains `doc_only / awaiting WS11` until `observability.recent` is exposed over MCP; no dashboard implementation is considered complete before that dependency lands. [OpenClaw](https://github.com/openclaw/openclaw)-inspired agent delegation, with architectural patterns borrowed from [Hermes Agent](https://github.com/NousResearch/hermes-agent).
...
- **WS13 — Web GUI Remote-First**: status `doc_only / awaiting WS11`. Architecture notes and schema/plumbing work may proceed, but the Phase 1 dashboard is blocked until WS11 exposes `observability.recent` over MCP. Do not treat this workstream as complete/designed or ship mock dashboard data before that dependency is verified.
